...
#include "StoreOperatorTranspose.h"
...
registerMooseObject("ImmersedBoundaryApp", StoreOperatorTranspose);
...
InputParameters validParams<StoreOperatorTranspose>()
{
    ...
    params.addRequiredParam<AuxVariableName>("fluid_variable",
                                             "The auxiliary variable to store the transferred values in.");
    params.addRequiredParam<VariableName>("solid_variable",
...
StoreOperatorTranspose::StoreOperatorTranspose(const InputParameters & parameters) :
GeneralUserObject(parameters),
_fe_problem(parameters.get< FEProblem *>("_fe_problem")),
_to_var_name(getParam<AuxVariableName>("fluid_variable")),
_from_var_name(getParam<VariableName>("solid_variable")),
...
void
StoreOperatorTranspose::initialize()
{
...
StoreOperatorTranspose::~StoreOperatorTranspose()
{}

void
StoreOperatorTranspose::execute()
{
    ...
    // _D = std::make_shared<SparseMatT>();
    
    MultiApp &  _multi_app = * _fe_problem->getMultiApp(_multiapp_name);
    
    FEProblemBase & from_problem = _multi_app.problemBase();  //SOLID
    
    FEProblemBase & to_problem = _multi_app.appProblemBase(0); //FLUID
...
void
StoreOperatorTranspose::assemble_mass_matrix(EquationSystems & es,const std::string & system_name, SparseMatT &D)
{
    //   // using namespace utopia;
    //
    ...
    //
    //    {
    //        utopia::Write<utopia::DSMatrixd> write(D);
    ...
    //
    //    utopia::DSMatrixd D_inv = utopia::local_sparse( matrix_A.m(), matrix_A.n(), 1);
    //
    //    {
    //        utopia::Write<utopia::DSMatrixd> write(D);
    //        for (auto it = mat_buffer.iter(); it; ++it) {
